...
##########################################################################
### This code is to run local fits in series,                   ##########
### Change the paramter MaxKinSets to adjust number of kinematic sets ####
##########################################################################
...
for j in range(1,MaxKinSets):
        # Load prediction inputs from CSV
    set_number = j  # You can specify the desired set number
    kin_df = df[df['set'] == set_number]
    kin_df = kin_df.reset_index(drop=True)
    #print(kin_df)
    create_folders('DNNmodels_Kin_Set_'+str(j))
    create_folders('Losses_Plots_Kin_Set_'+str(j))
    ...
    # ########## Evaluation ######
    # Load models
    models_folder = 'DNNmodels_Kin_Set_'+str(j)
