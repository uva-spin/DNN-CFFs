...
scratch_loction = '/scratch/cee9hc/DNN_CFFs/Tests/'
...
def GenerateReplicaData(df):
    ...
    # Sample F within its error
    tempF = np.array(df['F'])
    tempFerr = np.abs(np.array(df['errF']))  # Ensuring error is positive
    ReplicaF = np.random.normal(loc=tempF, scale=tempFerr)  # Generate F replica data

    # Print to ensure randomness in F generation
    print("Original F: ", tempF)
    print("Replica F:  ", ReplicaF)
