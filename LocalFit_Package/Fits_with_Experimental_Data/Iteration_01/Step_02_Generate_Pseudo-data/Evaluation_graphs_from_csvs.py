...
def generate_mean_std_plots(df, cff_labels, output_dir='CFF_Mean_Deviation_Plots', sets_per_plot=20):
    ...
    for cff in cff_labels:
        ...
        for chunk_start in range(0, len(sets), sets_per_plot):
            ...
            for i, (set_num, pred, std) in enumerate(zip(chunk_sets, chunk_predictions, chunk_stds)):
                # Plot the mean and standard deviation for each kinematic set in the chunk
                plt.plot([i + 1, i + 1], [pred - std, pred + std], color='blue', linewidth=2)  # Std bounds
                plt.scatter(i + 1, pred, color='blue', zorder=5)  # Mean as a dot

            # Set plot labels and title
            plt.title(f'{cff} Predictions with Standard Deviation (Sets {chunk_start + 1}-{chunk_end})')
            plt.xlabel('Kinematic Set')
            plt.ylabel(f'{cff} Prediction')
            ...
            plot_path = os.path.join(output_dir, f'{cff}_Prediction_Std_Plot_Sets_{chunk_start + 1}_to_{chunk_end}.png')
            plt.tight_layout()
...
cff_predictions_dir = '/scratch/qzf7nj/DNN_CFFs/LocalFit_BasicModel_Comparison_100_150/Sampled/'
