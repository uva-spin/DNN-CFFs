...
for j in available_kin_sets:
    ...
    models = [os.path.join(models_folder, f) for f in os.listdir(models_folder) if f.endswith('.keras')]
    ...
    for model_id in models:
        tempFLayer, tempCFFsLayer = load_FLayer_and_cffLayer(model_id)
        cffs, f_values = predict_cffs_and_f(tempCFFsLayer, tempFLayer, prediction_inputs)
        print(f"{model_id} -> cffs shape: {cffs.shape}, f_values shape: {f_values.shape}")

        # Ensure cffs always has shape (n_points, n_cffs)
        cffs = np.atleast_2d(cffs)
        if cffs.shape[0] == 1 and cffs.shape[1] != len(prediction_inputs):
            # If it's just (1, n_cffs), repeat for all points
            cffs = np.tile(cffs, (len(prediction_inputs), 1))

        cff_predictions.append(cffs)


    # Convert to arrays
    cff_predictions = np.array(cff_predictions)   # shape (n_models, n_points, n_cffs)
    f_predictions = np.array(f_predictions)       # shape (n_models, n_points, ?)
    ...
    for i, cff_label in enumerate(cff_labels):
        ...
        data = cff_predictions[:, :, i].flatten()
...
df_evaluation.to_csv("DNN_projections.csv", index=False)
