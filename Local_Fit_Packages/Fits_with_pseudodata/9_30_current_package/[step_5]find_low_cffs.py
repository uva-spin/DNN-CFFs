...
def summarize_low_residual_cffs_from_results(threshold=5):
    input_file = 'results.csv'
    output_summary = 'Summary_LowResidualCFFs_from_results.csv'
    output_allfour = 'Sets_with_AllFourCFFs.txt'
    ...
    for set_id in sorted(df['set'].unique()):
        ...
        for cff in cff_labels:
            res_col = f"{cff}_res"
            if res_col in row and pd.notnull(row[res_col]) and row[res_col] < threshold:
        ...
        if len(cffs_under_threshold) == len(cff_labels):
            all_four_sets.append(set_id)

    if summary_rows:
        result_df = pd.DataFrame(summary_rows)
        result_df.to_csv(output_summary, index=False)
        print(f" Saved: {output_summary} with {len(result_df)} rows")
    else:
        print(f"No residuals below {threshold} found in {input_file}")

    # Save all-four sets to txt as an array
    with open(output_allfour, "w") as f:
        f.write(str(all_four_sets))

    print(f" Saved sets with all 4 CFFs under threshold to {output_allfour}")
