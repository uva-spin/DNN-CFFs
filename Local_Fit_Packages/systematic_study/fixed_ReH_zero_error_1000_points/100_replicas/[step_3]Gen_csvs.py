...
from definitions import *  # Replaces BHDVCS_tf_modified - contains TotalFLayer needed for model loading
from user_inputs import *
from DNN_model import DNNmodel, FixedCFFsLayer  # Import FixedCFFsLayer for model loading
...
def load_FLayer_and_cffLayer(model_path):
    ...
    The model structure: inputs -> [ReH (DNN), ReE/ReHt/dvcs (FixedCFFsLayer)] -> cff_output_layer -> TotalFLayer
    """
    LayerF = tf.keras.models.load_model(
        model_path,
        custom_objects={
            'TotalFLayer': TotalFLayer,
            'FixedCFFsLayer': FixedCFFsLayer  # Needed for loading models with fixed CFFs
        }
    )
    
    # Extract the CFF output submodel
    # cff_output_layer contains [ReH, ReE, ReHt, dvcs] where:
    # - ReH comes from the DNN (trainable)
    # - ReE, ReHt, dvcs come from FixedCFFsLayer (fixed, calculated from KM15)
    LayerCFFs = tf.keras.Model(
        inputs=LayerF.input,
...
#     LayerF = tf.keras.models.load_model(model, custom_objects=custom_objects)
...
for j in available_kin_sets:
    ...
    for model_id in models:
        ...
        # Verify the CFF output layer shape
        # Should output [ReH, ReE, ReHt, dvcs] with shape (batch_size, 4)
        test_output = tempCFFsLayer.predict(prediction_inputs[:1], verbose=0)
        if test_output.shape[1] != 4:
            print(f"WARNING: Model {model_id} outputs {test_output.shape[1]} CFFs, expected 4!")
            print(f"  Output shape: {test_output.shape}")
            print(f"  First output: {test_output[0]}")
        
        cffs, f_values = predict_cffs_and_f(tempCFFsLayer, tempFLayer, prediction_inputs)
        
        # Verify CFFs shape: should be (batch_size, 4) for [ReH, ReE, ReHt, dvcs]
        if cffs.shape[1] != 4:
            print(f"ERROR: CFFs shape is {cffs.shape}, expected (N, 4)!")
    ...
    for i, cff_label in enumerate(cff_labels):
        ...
        # For fixed CFFs (ReE, ReHt, dvcs), they should be constant across replicas
        # since they're calculated from KM15, not trained
        if cff_label in ['ReE', 'ReHt', 'dvcs']:
            unique_values = np.unique(data)
            if len(unique_values) == 1:
                print(f"  ✓ Fixed CFF: All replicas have same value = {unique_values[0]:.6f}")
            else:
                print(f"  ⚠ WARNING: Fixed CFF has variation! Unique values: {unique_values}")
                print(f"    This shouldn't happen - fixed CFFs should be constant.")
